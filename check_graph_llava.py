...
model = LlavaForConditionalGeneration.from_pretrained(model_id, cache_dir="/data/sambit_phd/Llava", torch_dtype=torch.float16).eval()
model = model.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
processor = LlavaProcessor.from_pretrained(model_id, cache_dir="/data/sambit_phd/Llava")
...
# Prompts
prompts = [
    "USER: <image> Can you describe the graph in the given image? Please include number of node, number of edges, and everything you are able to notice from image.\nASSISTANT:",
...
def run_llava_and_save_jsonl_batch(excel_path, output_jsonl="llava_output.jsonl", batch_size=4):
    df = pd.read_excel(excel_path).head(n = 100)
...
# Example usage
run_llava_and_save_jsonl_batch(excel_path, "../DATA_SUBSET/llava_results_batch_lp.jsonl", batch_size=4)
